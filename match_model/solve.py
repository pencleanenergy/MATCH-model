...
import match_model
from match_model.utilities import (
...
def main(args=None, return_model=False, return_instance=False):
    ...
    with LogOutput(logs_dir):
        ...
        if model.options.verbose:
            print("\n=======================================================================")
            print("MATCH {}".format(match_model.__version__))
            print("=======================================================================")
            print("Arguments:")
...
def iterate(m, iterate_modules, depth=0):
    ...
    if depth == len(iterate_modules):
        ...
        # This prepends 'match_model.' if needed, to be consistent with modules.txt.
        current_modules = [
            sys.modules[module_name if module_name in sys.modules else 'match_model.' + module_name]
...
def define_arguments(argparser):
    ...
    argparser.add_argument("--solver-suffixes", default='',
        help="Extra suffixes to add to the model and exchange with the solver (e.g., iis, rc, dual, or slack)")
    ...
    # e.g. by defining them in a define_standard_arguments() function in match.utilities.py
...
def get_module_list(args):
    ...
    if module_list_file is None:
        # note: this could be a RuntimeError, but then users can't do "match solve --help" in a random directory
    ...
    modules.append("match_model.solve")
...
# if we want to make these standard parts of MATCH.
