...
def create_model(module_list=None, args=sys.argv[1:]):
    ...
    Construct a Pyomo AbstractModel using the MATCH modules or packages
    ...
    match modules, so parse_args() generates an error. This behavior can be
    ...
    if module_list is None:
        ...
        module_list = match_model.solve.get_module_list(args)
...
def post_solve(instance, outputs_dir=None):
    ...
    inputs_dir = getattr(instance.options, "inputs_dir", "inputs")
    ...
    for module in instance.get_modules():
        if hasattr(module, "post_solve"):
            try:
                ...
                module.post_solve(instance, outputs_dir, inputs_dir)
...
def load_aug(match_data, optional=False, auto_select=False, optional_params=[], **kwds):
    """
    ...
    if suffix in {"tab", "tsv"}:
        ...
        raise match_model.utilities.InputError(
    ...
    if "index" in kwds:
        ...
        if isinstance(kwds["index"], list):
            ...
            print(
                f'WARNING: Please manually set the number of dimensions for Set {kwds["index"]}'
            )
            num_indexes = 1
