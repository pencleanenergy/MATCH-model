...
def define_components(mod):
    ...
    #TODO: Load this data
    mod.social_cost_of_carbon = Param(mod.PERIODS,
    ...
    mod.GenDirectConsequentialEmissionsInTP = Expression(
        mod.ADDITIONAL_GENS, mod.TIMEPOINTS,
    ...
    mod.GenIndirectConsequentialEmissionsInTP = Expression(
        mod.ADDITIONAL_GENS, mod.TIMEPOINTS,
        rule=lambda m, g, t: -1 * (m.TotalGen[g,t] * m.lrmer[m.gen_cambium_region[g],t]))

    mod.StorageIndirectConsequentialEmissionsInTP = Expression(
        mod.ADDITIONAL_STORAGE_GENS, mod.TIMEPOINTS,
        rule=lambda m,g,t: (m.ChargeStorage[g,t] - m.DischargeStorage[g,t]) * m.lrmer[m.gen_cambium_region[g],t])      

    # Calculate total emissions   
    def TotalEmissions_rule(m,g,t):
        totalemissions = 0
        if g in m.ADDITIONAL_GENS:
            totalemissions = totalemissions + m.GenDirectConsequentialEmissionsInTP[g,t] + m.GenIndirectConsequentialEmissionsInTP[g,t]
        if g in m.ADDITIONAL_STORAGE_GENS:
            totalemissions = totalemissions + m.StorageIndirectConsequentialEmissionsInTP[g,t]
        return totalemissions
    mod.GenTotalConsequentialEmissionsInTP = Expression(
        mod.GEN_TPS,
        rule=TotalEmissions_rule)
    ...
    mod.GenEmissionsCostInTP = Expression(
        mod.TIMEPOINTS,
        rule=lambda m,t: sum(m.social_cost_of_carbon[m.tp_period[t]] * m.GenTotalConsequentialEmissionsInTP[g,t] for g in m.GENERATION_PROJECTS))
...
def load_inputs(mod, match_data, inputs_dir):
    ...
    match_data.load_aug(
        ...
        param=[mod.gen_emission_factor, mod.gen_is_additional, mod.gen_ccs_energy_load, mod.gen_ccs_capture_efficiency, mod.gen_cambium_region])
