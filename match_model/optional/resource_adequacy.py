...
def define_components(mod):
    ...
    def CalculateEffectiveELCC(m,g,p,mo):
        ...
        if m.gen_is_ra_eligible[g]:
            if m.gen_is_variable[g] and not m.gen_is_hybrid[g]:
                ...
                storage_hybrid_capacity_ratio = (m.storage_hybrid_min_capacity_ratio[hybrid_gen_storage_component] + m.storage_hybrid_max_capacity_ratio[hybrid_gen_storage_component]) / 2
                effective_elcc = (m.elcc[p, m.gen_energy_source[g], mo] * 
                                      ((m.ra_production_factor[p, m.gen_energy_source[g], mo] - min(storage_hybrid_capacity_ratio * m.storage_energy_to_power_ratio[hybrid_gen_storage_component],m.ra_production_factor[p, m.gen_energy_source[g], mo]))
    ...
    mod.GeneratorELCC = Expression(
        mod.GENERATION_PROJECTS, mod.PERIODS, mod.MONTHS,
        rule=CalculateEffectiveELCC)
    ...
    def CalculateAvailableRACapacity(m,p,mo):
        ...
        for g in m.GENERATION_PROJECTS:
            ...
            if m.gen_is_hybrid[g] and m.gen_is_storage[g]:
                storage_hybrid_capacity_ratio = (m.storage_hybrid_min_capacity_ratio[g] + m.storage_hybrid_max_capacity_ratio[g]) / 2
                system_ra_capacity = system_ra_capacity + (m.GeneratorELCC[g,p,mo] * (m.GenCapacity[g,p] / storage_hybrid_capacity_ratio))
            else:
                system_ra_capacity = system_ra_capacity + (m.GeneratorELCC[g,p,mo] * m.GenCapacity[g,p])
        return system_ra_capacity
    mod.AvailableRACapacity = Expression (
        mod.PERIODS, mod.MONTHS,
        rule=CalculateAvailableRACapacity)
    ...
    mod.GeneratorFlexRAValue = Expression(
        ...
        rule=CalculateEffectiveFlexibleCapacity)

    mod.AvailableFlexRACapacity = Expression (
        mod.PERIODS,
        rule=lambda m,p: sum(m.GeneratorFlexRAValue[g,p] for g in m.GENERATION_PROJECTS))
...
def load_inputs(mod, match_data, inputs_dir):
    ...
    match_data.load_aug(
        ...
        select=('period','month', 'flexible_ra_requirement','flexible_ra_cost','flexible_ra_resell_value'),
