...
parser = _ArgumentParser(
    allow_abbrev=False, description="Solve one or more MATCH scenarios."
)
parser.add_argument(
    "--scenario",
    "--scenarios",
...
# pid (useful when launching several instances of `match solve-scenarios` in
...
if job_id is None:
    job_id = os.environ.get("MATCH_JOB_ID")
if job_id is None:
        ...
        if all(all_done):
            ...
            for s in scenarios:
                ...
                predetermined_list = list(
                    pd.read_csv(predetermined, usecols=["GENERATION_PROJECT"])[
                        "GENERATION_PROJECT"
                    ]
                )
                ...
                if i == 0:
                    ...
                    capacity_df = pd.read_csv(
                        capacity_file,
                        usecols=["generation_project", "gen_tech", "GenCapacity"],
                    ).rename(columns={"GenCapacity": s})
                    capacity_df["predetermined"] = "No"
                    capacity_df.loc[
                        capacity_df["generation_project"].isin(predetermined_list),
                        "predetermined",
                    ] = "Yes"
...
def scenarios_to_run():
    ...
    if requested_scenarios:
        ...
        while not all_done:
            ...
            already_run = {
                f for f in os.listdir(scenario_queue_dir) if os.path.isdir(f)
            }
...
def is_verbose(scenario_args):
    ...
    # note: this duplicates settings in match_model.solve, so it may fall out of date
