...
depend on multiple MATCH modules. Each individual MATCH module
that defines components should only access model components that
it defined or that were defined upstream in MATCH modules that
...
from match_model.utilities import string_types
dependencies = 'match_model.financials'
...
from match_model.utilities import make_iterable
...
def write_table(instance, *indexes, **kwargs):
    ...
    with open(output_file, 'w') as f:
        w = csv.writer(f, dialect="match-csv")
        # write header row
        ...
        try:
            ...
            print("DEPRECATION WARNING: match_model.reporting.write_table() was called with a function")
            print("that expects multidimensional index values to be stored in tuples, but MATCH now unpacks")
...
def save_generic_results(instance, outdir, sorted_output):
    ...
    for var in components:
        ...
        with open(output_file, 'w') as fh:
            writer = csv.writer(fh, dialect='match-csv')
            if var.is_indexed():
