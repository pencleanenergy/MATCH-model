...
def define_components(mod):
    ...
    mod.Enforce_Hybrid_Build = Constraint(
        ...
        rule = lambda m, g, p: (m.BuildMinGenCap[g,p] if g in m.HYBRID_STORAGE_GENS else m.BuildMinGenCap[g,p])  == 
        (m.BuildMinGenCap[m.storage_hybrid_generation_project[g],p] if g in m.HYBRID_STORAGE_GENS else m.BuildMinGenCap[g,p])
        )
    """
    ...
    """
    # Specify the cost of adding storage capacity and add to objective function
    mod.StorageCapacityCosts = Expression(
        mod.STORAGE_GENS, mod.PERIODS,
        rule=lambda m, g, p: sum(
            m.BuildStorageEnergy[g, bld_yr] * m.ppa_capacity_cost[g]
            for bld_yr in m.BLD_YRS_FOR_GEN_PERIOD[g, p]))

    mod.TotalStorageCapacityCosts = Expression(
        mod.PERIODS,
        rule=lambda m, p: sum(
            m.StorageCapacityCosts[g, p]
            for g in m.STORAGE_GENS))
    mod.Cost_Components_Per_Period.append('TotalStorageCapacityCosts')
    """

    """
    #specify energy costs of storage and add to objective function
    mod.StorageEnergyCostsInTP = Expression(
        mod.TIMEPOINTS,
        rule=lambda m, t: sum(
            m.DispatchGen[g, t] * m.ppa_energy_cost[g, p]
            for g in m.STORAGE_GENS_IN_PERIOD[m.tp_period[t]] for p in m.PERIODS))
    mod.Cost_Components_Per_TP.append('StorageEnergyCostsInTP')
    """
    ...
    def Charge_Storage_Upper_Limit_rule(m, g, t):
        return m.ChargeStorage[g,t] <= \
            m.DispatchUpperLimit[g, t] * m.storage_charge_to_discharge_ratio[g]
    mod.Charge_Storage_Upper_Limit = Constraint(
        mod.STORAGE_GEN_TPS,
        rule=Charge_Storage_Upper_Limit_rule)

    def Charge_Hybrid_Storage_Upper_Limit_rule(m, g, t):
        return m.ChargeStorage[g,t] <= m.DispatchGen[m.storage_hybrid_generation_project[g],t]
    mod.Charge_Hybrid_Storage_Upper_Limit = Constraint(
        mod.HYBRID_STORAGE_GEN_TPS,
        rule=Charge_Hybrid_Storage_Upper_Limit_rule)
    ...
    def Track_State_Of_Charge_rule(m, g, t):
        return m.StateOfCharge[g, t] == \
            ...
            (m.ChargeStorage[g, t] * m.storage_roundtrip_efficiency[g] -
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.data()['STORAGE_GENS'] = {
        None: list(switch_data.data(name='storage_roundtrip_efficiency').keys())}
