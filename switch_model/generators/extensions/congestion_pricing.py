"""
Define an energy demand cost per tp based on the load and DLAP
Define delivery_cost for each generator 
Define pnode_revenue for each generator
Define congestion cost as delivery_cost - pnode_revenue

In order to calculate a congestion cost for each generator, we need to calculate the delivery cost of any excess generation
In reality, we would not take delivery of excess generation, so we only pay the DLAP cost of any load
"""
